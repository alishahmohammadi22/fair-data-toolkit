  ...
  <div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
      ...
      <div class="flex items-center gap-3">
        <transition name="fade">
          ...
          <span v-else-if="saved" class="text-sm text-emerald-600 font-medium">✓ Saved</span>
        </transition>
    ...
    <div class="border-b border-gray-200 mb-6">
      <nav class="flex">
        ...
        >
          <span :class="['w-2 h-2 rounded-full', TAB_COLORS[p].dot]"></span>
          <span class="font-semibold">{{ p }}</span>
          <span class="hidden sm:inline text-xs opacity-75">{{ LABELS[p] }}</span>
    ...
    <!-- Indicator cards -->
    <div v-if="tabIndicators.length && Object.keys(scores).length" class="space-y-3">
      <IndicatorCard
    ...
    <div v-else class="text-center text-gray-400 py-10">Loading indicators…</div>
  </div>
</template>
